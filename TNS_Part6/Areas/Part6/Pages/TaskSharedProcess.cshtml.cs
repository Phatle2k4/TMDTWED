...
namespace TNS_Task.Areas.Tasks.Pages
{
    public class TaskSharedProcessModel : PageModel
    {
        ...
        [BindProperty]
        public TaskRecord TodoRequest { get;private set; }
        public TaskRecord TodoReponse { get; set; }
        public List<TaskRecord> ListReponse { get; set; }    
        public IActionResult OnGet(string key)
        {
            CheckAuth();
            TodoReponse = new Models.TaskRecord();
            if (key.Length == 36)
            {
                TodoRequest = new Models.TaskRecord(key);
                ListReponse = AccessData.TaskOfParent_ListFull(key);
            }
            else
            {
                TodoRequest = new Models.TaskRecord();
